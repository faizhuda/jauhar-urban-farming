...
- [ ] Setelah domain aktif: update `site` di `astro.config.mjs`, `SITE.url` di `src/config.ts`, `Sitemap:` di `public/robots.txt` — ketiganya **harus identik persis**
...
- [ ] Setelah foto asli masuk: hapus baris terkait di kredit foto stok, jalankan Rich Results Test ulang, re-audit PageSpeed di domain final
...
- [ ] Konfirmasi harga final & keputusan: pickled cucumber / cucumber chips / garden salad pack / cucumber seedlings — lanjut dijual beneran atau tetap eksperimen?
